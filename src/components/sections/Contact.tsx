...
type Phase = 'IDLE' | 'SCROLLING' | 'EXPANDING' | 'TEXT_SCROLLING' | 'FORM_ACTIVE' | 'CLOSING';
...
export default function Contact() {
  ...
  const originRef = useRef<{
    ...
    btnCenterX: number;
    btnCenterY: number;
  }>({ titleTop: 0, titleLeft: 0, titleWidth: 0, btnCenterX: 0, btnCenterY: 0 });
  ...
  const runDialogueAnimation = () => {
    ...
    tl.add(() => {
      ...
      if (line1Ref.current && line2Ref.current && line1Old && line2Old) {
        ...
        const currentScale = (gsap.getProperty(titleRef.current, "scale") as number) || 1;
        
        gsap.fromTo(line1Ref.current, 
          { x: (line1Old.left - line1New.left) / currentScale, y: (line1Old.top - line1New.top) / currentScale },
          { x: 0, y: 0, duration: 1.0, ease: 'power3.inOut' }
        );
        gsap.fromTo(line2Ref.current, 
          { x: (line2Old.left - line2New.left) / currentScale, y: (line2Old.top - line2New.top) / currentScale },
          { x: 0, y: 0, duration: 1.0, ease: 'power3.inOut' }
        );
      }
    }, "-=1.0");
  ...
  const closeForm = () => {
    ...
    const closeTl = gsap.timeline({
      onComplete: () => {
        ...
        if (overlayRef.current) {
          gsap.set(overlayRef.current, { display: 'none', scale: 0, clearProps: "all" });
        }
    ...
    if (titleRef.current && originRef.current.titleTop) {
      ...
      closeTl.add(() => {
        ...
        titleRef.current.style.width = originRef.current.titleWidth ? `${originRef.current.titleWidth}px` : '';
        ...
        gsap.fromTo(line1Ref.current,
          { x: (line1Before.left - line1After.left) / currentScale, y: (line1Before.top - line1After.top) / currentScale },
          { x: 0, y: 0, duration: 1.0, ease: 'power3.inOut' }
        );
        gsap.fromTo(line2Ref.current,
          { x: (line2Before.left - line2After.left) / currentScale, y: (line2Before.top - line2After.top) / currentScale },
          { x: 0, y: 0, duration: 1.0, ease: 'power3.inOut' }
        );
      }, 0.6);
    }
    ...
    <section ref={sectionRef} className={styles.contact} id="contact">
      <div className={styles.container}>
        ...
        <h2 ref={titleRef} className={styles.title}>
          <span ref={line1Ref} style={{ display: 'inline-block' }}>Let's build something</span>
          <br/>
          <span className="titleSpace" style={{display: 'none'}}>&nbsp;</span>
          <span ref={line2Ref} style={{ display: 'inline-block' }}>people actually remember.</span>
        </h2>
